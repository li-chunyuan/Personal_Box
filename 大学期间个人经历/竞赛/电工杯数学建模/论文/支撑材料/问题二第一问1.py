...
# 创建LP问题
prob = pulp.LpProblem("Maximize_Amino_Acid_Score", pulp.LpMaximize)
...
# 每种食物最多只能购买5次的约束
for food in foods:
    prob += x[food] <= 3, f"Max_Purchase_{food}"
...
# 目标函数：最大化蛋白质氨基酸评分
prob += pulp.lpSum([x[food] * data_aggregated.loc[data_aggregated["食物名称"] == food, "氨基酸评分"].values[0] for food in foods]), "Total_Amino_Acid_Score"
...
print(f"Total Amino Acid Score: {pulp.value(prob.objective)}")
