...
% =============================================================
% 1. 插入 C 语言代码（默认样式）：
% \begin{lstlisting}[caption={这是一个 C 语言示例}, label={code:c_example}]
% #include <stdio.h>
% int main() {
%     printf("Hello, World!\n");
%     return 0;
...
% \end{lstlisting}
%
% 2. 插入 Python 代码（显式指定样式）：
% \begin{lstlisting}[style=python, caption={这是一个 Python 示例}, label={code:py_example}]
% def hello():
%     print("Hello, World!")
% \end{lstlisting}
%
% 3. 插入外部代码文件：
% \lstinputlisting[style=cstyle, caption={从文件加载}, label={code:external_file}]{src/main.c}
...
% ⑧ 参考文献支持类
% =============================================================
% style=ieee : 采用 IEEE 标准格式（电子信息/计算机工程领域通用标准）
% backend=biber : 使用 biber 后端处理文献（比 bibtex 更现代，支持 utf8）
% sorting=none : 按在正文中引用的顺序排序（[1]出现在第一处，[2]出现在第二处...）
%                如果不加这一项，默认可能会按作者首字母排序，这不符合工程类论文规范。
