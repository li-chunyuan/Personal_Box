...
# 创建LP问题
prob = pulp.LpProblem("Optimize_Amino_Acid_Score_and_Cost", pulp.LpMinimize)
...
cost = pulp.lpSum([x[food] * data_aggregated.loc[data_aggregated["食物名称"] == food, "价格（元/份）"].values[0] for food in foods])
amino_acid_score = pulp.lpSum([x[food] * data_aggregated.loc[data_aggregated["食物名称"] == food, "氨基酸评分"].values[0] for food in foods])
prob += alpha * cost - (1 - alpha) * amino_acid_score, "Combined_Objective"
...
# 蛋白质、脂肪、碳水化合物比例约束
prob += pulp.lpSum([x[food] * data_aggregated.loc[data_aggregated["食物名称"] == food, "蛋白质 (g/100g)"].values[0] * data_aggregated.loc[data_aggregated["食物名称"] == food, "可食部（克/份）"].values[0] / 100 for food in foods]) >= 0.10 * 2400 / 4, "Min_Protein"
